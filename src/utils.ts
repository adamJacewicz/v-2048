...
export const isInRange = (value: number, end = BOARD_SIZE, start = 0) => {
...
export const allPositions = Array.from({ length: BOARD_SIZE ** 2 }, (_, i) =>
  getCoordinates(i)
)

export const findNeighbors = (tiles: Tile[], tile: Tile, axis: Axis) => {
  const sameAxisTiles = tiles.filter((t) => t[axis] === tile[axis])
  const tileIndex = sameAxisTiles.findIndex(({ id }) => id === tile.id)

  return [sameAxisTiles[tileIndex - 1], sameAxisTiles[tileIndex + 1]].filter(
