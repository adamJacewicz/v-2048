...
export const useGame = createSharedComposable(() => {
  const state = useLocalStorage<GameState>("2048", getInitialState(), {
    mergeDefaults: true,
  })
  ...
  const move = (key: MoveKeyType) => {
    ...
    const diff = useArrayDifference<Tile>(cloned.value, tiles.value, (a, b) => {
      const keys = Object.keys(a) as (keyof Tile)[]
  ...
  const isMergePossible = computed(() =>
    tiles.value
      ...
      .some((tile, _, array) => {
        const neighbors = [
          ...findNeighbors(array, tile, Axis.Y),
          ...findNeighbors(array, tile, Axis.X),
        ]
