...
vi.mock("../constants.ts", async () => {
  return {
    ...((await vi.importActual("../constants.ts")) as Promise<{}>),
    BOARD_SIZE: 4,
  }
})
...
describe("inRange", () => {
  it("returns false if value is not in range", () => {
    ...
    expect(inRange(5, 4, 5)).toBe(false)
    expect(
      inRange(
        new Date(2020, 5, 12),
        new Date(2020, 6, 12),
    ...
    expect(inRange("d", "e", "h")).toBe(false)
  })
  ...
  it("returns true if value is in range", () => {
    ...
    expect(inRange("d", "a", "h")).toBe(true)
  })
})
